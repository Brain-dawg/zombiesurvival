...
if SERVER then
	...
	local function RequestScreen( pl, cmd, args )
		if not IsValid(pl) or not pl:IsAdmin() or not args then
			return
		end
...
if CLIENT then
	...
	net.Receive( "RequestScreen", function( len )
		...
		--I suggest to avoid setting quality bigger than 15, otherwise it has 20% chance of not working on huge screens
