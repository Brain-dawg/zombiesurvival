function CheckPlayerDeathNitwits ( pl, attacker, dmginfo )
...
--[[
		...
		for i = 1, #HumanClasses do
			if k == HumanClasses[i].Name then
				...
				if CurrentPoint > MaximumPoint then
					...
					if k == "Berseker" then
						MostChosenHumanClass = "Berserker"
					end
				end
			end
		end
	...
	umsg.Start("RecMostChosenClass", to)
		umsg.String ( MostChosenUndeadClass )
	umsg.End()
...
function GM:OnEndRound ( winner )
	...
	for _, pl in pairs(player.GetAll()) do
		if pl:Team() == TEAM_HUMAN then
			...
			--if team.NumPlayers (TEAM_HUMAN) + team.NumPlayers(TEAM_UNDEAD) > 8 then
	...
	--- Write server data
	---GAMEMODE:UpdateServerStats()
	--GAMEMODE:WriteServerData()
	
--	if BUG_REPORT then
	--	GAMEMODE:BugsWriteFile()
	--end
