...
-- Render hook
function ENT:Draw()
	self:SetColor( Color(30, 240, 30, 195) )
	
	-- Don't draw on self
	if self:GetParent() ~= MySelf then self:DrawModel() end
	...
	self.Particles[1]:SetColor( math.random( 70, 90 ), math.random( 70, 150 ), math.random( 30, 70 ) ) 
	...
	self.Particles[2]:SetColor( math.random( 10, 30 ), math.random( 70, 120 ), 30 ) 
	
	-- While flying
	if not self.Sticked then
		self.PuffTimer = CurTime() + 0.025
	end
end
